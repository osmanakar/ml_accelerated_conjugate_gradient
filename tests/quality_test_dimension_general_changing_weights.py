...
project_folder_subname = "PFI_MLV20_T1_eigvector_random_N63"
...
w1 = ml_model.get_weights()
#%%
for i in range(3,7):
    w1[2*i] = w1[2*i]/2
#%%
ml_model.set_weights(w1)

#%% Creating new model
d_type_ = tf.float16
fil_num=32
fil_dim = 3
input_rhs = keras.Input(shape=(dim, dim, 1), dtype=d_type_)
first_layer = layers.Conv2D(32, (fil_dim, fil_dim), activation='linear', padding='same',dtype=d_type_)(input_rhs)
la = layers.Conv2D(fil_num, (fil_dim, fil_dim), activation='relu', padding='same',dtype=d_type_)(first_layer)
for i0 in range(16):
    lb = layers.Conv2D(fil_num, (fil_dim, fil_dim), activation='relu', padding='same',dtype=d_type_)(la)
    la = layers.Conv2D(fil_num, (fil_dim, fil_dim), activation='relu', padding='same',dtype=d_type_)(lb) + la
