...
with open(project_data_folder+'A_cos_vec_10000_N'+str(dim-1)+'.npy', 'rb') as f:
    ritz_vectors = np.load(f)

#cos_vec = cos_vec[::-1]

"""
ritz_vectors = np.zeros([10000,dim2])
for i in range(10000):
    ritz_vectors[i] = A_sparse.dot(cos_vec[i])
    ritz_vectors[i] = ritz_vectors[i]/np.linalg.norm(ritz_vectors[i])
"""    
print(sum(sum(ritz_vectors[0:10000-100])))
print("Computing Ritz Values")
ritz_values = CG.create_ritz_values(ritz_vectors)
print("ritz_vals[0:50]",ritz_values[0:50])
print("ritz_vals[9950:10000]",ritz_values[9950:10000])
...
for it in range(0,100):
    ...
    b_rhs[l_b:r_b] = mat_mult(ritz_vectors[0:num_ritz_vectors-num_zero_ritz_vals].transpose(),coef_matrix).transpose()
...
#%%
with open(project_data_folder+'b_rhs_20000_10000_A_cos_vectors_V2_for_3D_random_N'+str(dim-1)+'.npy', 'wb') as f:
    np.save(f, b_rhs)
#%%
"""
